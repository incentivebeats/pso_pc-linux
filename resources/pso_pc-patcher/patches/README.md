...
Also be warned that since PSO PC uses a patch server, your patches may be overwritten depending on what the server forces downstream (this can include overwriting .exe files and other files related to graphics, drops, enemies, etc.,). If you are unsure if something is allowed or will work with the server you play on, ASK THE ADMINS.  
...
- No more glitched out dragon for certain renderers. 
- This patch will work and is allowed on Sylverant's pso.exe
- This patch is already on the patch menu for Peeps
- This patch will not work on Ragol
